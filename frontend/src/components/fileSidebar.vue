<template>
  <li>
    <a href="#" v-on:click="file_flag = !file_flag">
      <i class="fas fa-file sidebar_icons"></i>
      <strong>File</strong>
      <i
        class="fas downmenu_icon"
        v-bind:class="{toggle_dropdown_menu : !file_flag, 'fa-caret-down': !sidebar_toggle}"
      ></i>
    </a>
    <ul class="list-unstyled" id="file" v-bind:class="{collapse: file_flag}">
      ...
      <li>
        <a href="#" v-on:click="local_vtp_open">
          Open
...
export default {
  ...
  methods: {
    remove_output: function() {
      ...
      if (outDiv.children[1].children.length) {
        outDiv.removeChild(outDiv.getElementsByTagName("div")[1]);
        let a = document.createElement("div");
        a.setAttribute("class", "content");
        outDiv.insertBefore(a, outDiv.lastChild);
        outDiv.style.backgroundColor = "#fff";
      }
      //document.getElementById("softwareOutput").style.backgroundImage = "url('../images/p.png')";
      document.getElementsByClassName("welcome_div")[0].style.display = "block";
    },
